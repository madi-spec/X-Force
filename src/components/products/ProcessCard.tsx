...
function DaysIndicator({ days, status }: { days: number; status: HealthStatus }) {
  const getColor = () => {
    if (status === 'stalled') return 'text-[#ef4444]';
    if (status === 'attention') return 'text-[#f59e0b]';
    ...
    >
      ...
      <div className="flex items-center justify-between pt-2 border-t border-[#e6eaf0]">
        <div className="flex items-center gap-3">
          <DaysIndicator days={item.days_in_stage} status={item.health_status} />
