...
export function FullAnalysisModal({
  ...
  // Normalize key_insights (can be keyInsights or key_insights)
  const keyInsights = analysis.key_insights || analysis.keyInsights || [];
  const signals = analysis.signals || [];
  const topics = analysis.topics || [];
  const speakerBreakdown = analysis.speaker_breakdown || [];
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeSection === 'topics' && (
            <div className="space-y-3">
              {topics.length > 0 ? (
                topics.map((topic, i) => {
                  // Handle both string[] and Topic[] formats
                  const topicName = typeof topic === 'string' ? topic : topic.name;
                  const topicMentions = typeof topic === 'string' ? null : topic.mentions;
                  const topicRelevance = typeof topic === 'string' ? 0.5 : topic.relevance;
          ...
          {activeSection === 'speakers' && (
            <div className="space-y-3">
              {speakerBreakdown.length > 0 ? (
                speakerBreakdown.map((speaker, i) => (
                  <div
                    key={i}
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          <p className="text-xs text-gray-400 text-center">
            ...
            {analysis.analyzed_at ? new Date(analysis.analyzed_at).toLocaleDateString() : 'Unknown date'}
          </p>
        </div>
      </div>
    </div>
